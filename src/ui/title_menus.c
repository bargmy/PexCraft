...
static void draw_title_logo_3d(float partial) {
    ...
    gluPerspective(70.0, (double)g_win_w / (double)title_h, 0.05, 100.0);
    glViewport(0, g_win_h - title_h, g_win_w, title_h);
...
static void draw_options_screen(void) {
    draw_default_bg();
    draw_centered_text("Options", g_gui_w / 2, 20, 16777215);
    draw_all_buttons();
}

static void draw_controls_screen(void) {
    draw_default_bg();
    draw_centered_text("Controls", g_gui_w / 2, 20, 16777215);
...
static void draw_multiplayer(void) {
    ...
    draw_text("PEXCRAFT Multiplayer is currently not finished, but there", g_gui_w / 2 - 140, g_gui_h / 4 - 60 + 60 + 0, 10526880);
    draw_text("is some buggy early testing going on.", g_gui_w / 2 - 140, g_gui_h / 4 - 60 + 60 + 9, 10526880);
    draw_text("Enter the IP of a server to connect to it:", g_gui_w / 2 - 140, g_gui_h / 4 - 60 + 60 + 36, 10526880);
    int x = g_gui_w / 2 - 100;
    int y = g_gui_h / 4 - 10 + 50 + 18;
    draw_rect(x - 1, y - 1, x + 201, y + 21, -6250336);
    draw_rect(x, y, x + 200, y + 20, -16777216);
    char field[80];
    snprintf(field, sizeof(field), "%s%s", g_multiplayer_ip, (g_ticks / 6 % 2 == 0) ? "_" : "");
