/* Linux/SDL2 Minecraft Classic resources installer.
   Uses libcurl for HTTPS downloads and the internal ZIP/JAR extractor in
   pxc_zip_extract.c. Textures come from b1.0 client.jar; sound effects come
   from the official b1.0 legacy.json asset index, excluding music/records. */
...
static void classic_install_fail(const char *msg) {
    ...
    log_msg("Minecraft Classic resources install failed: %s", g_classic_install_error);
}
...
static int classic_download_client_jar_linux(const char *url, const char *zip_path) {
    ...
    if (!out) { classic_install_fail("Could not create downloaded client file"); return 0; }
    curl_global_init(CURL_GLOBAL_DEFAULT);
    curl = curl_easy_init();
    ...
    if (write_ctx.downloaded < 1024) { DeleteFileA(zip_path); classic_install_fail("Downloaded client.jar was empty"); return 0; }
...
static int classic_download_legacy_sounds(void) {
    ...
    unsigned long long total = 0;
    char root[MAX_PATHBUF], marker[MAX_PATHBUF];
    int ok;
    ...
    for (int i = 0; i < count; ++i) {
        char out_path[MAX_PATHBUF], url[256], st[MAX_LABEL];
        int pct = 5 + (int)(((unsigned long long)i * 90ULL) / (unsigned long long)(count > 0 ? count : 1));
        pxc_zip_make_output_path(out_path, sizeof(out_path), root, assets[i].path);
        if (classic_file_size_bytes(out_path) == assets[i].size) { downloaded++; continue; }
