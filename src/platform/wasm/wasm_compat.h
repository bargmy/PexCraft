#ifndef PEXCRAFT_WASM_COMPAT_H
#define PEXCRAFT_WASM_COMPAT_H
...
/* WebGL/OpenGL ES 2 has no fixed-function normal call. */
...
/* Directory enumeration for the simple path-glob patterns used by this project. */
...
/* Browser build is intentionally cooperative/single-threaded.  Returning NULL
   from CreateThread activates the existing synchronous fallbacks in worldgen,
   saving, rendering, and shutdown code without requiring SharedArrayBuffer. */
typedef struct CRITICAL_SECTION { int unused; } CRITICAL_SECTION;
static inline void InitializeCriticalSection(CRITICAL_SECTION *cs) { if (cs) cs->unused = 0; }
...
static inline HANDLE CreateThread(void *sa, size_t stack, LPTHREAD_START_ROUTINE fn, LPVOID arg, DWORD flags, DWORD *tid) {
    (void)sa; (void)stack; (void)fn; (void)arg; (void)flags; if (tid) *tid = 0; return NULL;
}
...
static inline DWORD WaitForSingleObject(HANDLE h, DWORD ms) {
    ...
    if (h->kind == PEX_HANDLE_EVENT && h->signaled) {
...
static inline void Sleep(DWORD ms) { if (ms) SDL_Delay(ms); }
...
static inline DWORD GetTickCount(void) { return (DWORD)((uint64_t)emscripten_get_now() & 0xffffffffu); }
...
static inline DWORD GetModuleFileNameA(void *module, char *buf, DWORD cap) { (void)module; if (buf && cap) { snprintf(buf, cap, "pexcraft_wasm"); return (DWORD)strlen(buf); } return 0; }
