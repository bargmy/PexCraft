/* Android TV native platform filesystem/timing layer.
   Keep this isolated from the desktop SDL2 filesystem so TV app storage rules
   can evolve without changing Linux/Windows/PSP targets. */
...
static const char *android_tv_data_base(void) {
    const char *base = SDL_AndroidGetInternalStoragePath();
    if (base && base[0]) return base;
    base = getenv("PEXCRAFT_HOME");
    if (base && base[0]) return base;
    return ".";
}

static void init_dirs(void) {
    const char *base = android_tv_data_base();

    /* Java copies bundled APK assets to <files>/assets before SDL starts.
       Changing cwd lets the existing asset loader keep using "assets/..." paths. */
    if (base && base[0]) chdir(base);

    snprintf(g_mc_dir, sizeof(g_mc_dir), "%s/.pexcraft", base);
    ensure_dir(g_mc_dir);
    ...
    PEX_ANDROID_LOGI("PEXCRAFT Android TV data dir: %s", g_mc_dir);
}
