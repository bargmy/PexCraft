/* Android classic texture pack installer stub.
...
static void classic_install_fail(const char *msg) {
    lstrcpynA(g_classic_install_error, msg ? msg : "Unsupported on Android build", sizeof(g_classic_install_error));
...
static void classic_resource_size_format(char *out, size_t cap) {
    snprintf(out, cap, "not available in this Android build");
}

static void start_classic_pack_install(void) {
    classic_install_fail("Runtime client.jar download is disabled for Android. Bundle or copy a texture pack instead.");
}
...
static void classic_pack_install_tick(void) {
    ...
    if (state == CLASSIC_INSTALL_ERROR) {
        ...
        open_notice("Texture Pack", "Classic pack download is disabled on Android for now.", g_classic_install_error);
    }
}
