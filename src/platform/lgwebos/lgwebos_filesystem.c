/* LG webOS native platform filesystem/timing layer.
...
static const char *lgwebos_data_base(void) {
    const char *v = getenv("PEXCRAFT_HOME");
    if (v && v[0]) return v;
...
static void init_dirs(void) {
    const char *base = lgwebos_data_base();
    ...
    fprintf(stderr, "PEXCRAFT LG webOS data dir: %s\n", g_mc_dir);
}
