...
| `docs/worldgen_migration_status.md` | Updated this report with the ID/texture audit and decoration status. |
...
| Added-block world textures | Every block added in the 1.2.5 ranges `21-36` and `92-124` has a `block_texture_resolve()` mapping. |
...
| Area | Change |
|---|---|
| Golden chunk dump harness | Added `tools/worldgen_dump.c`, a standalone C harness that emits SHA-1 hashes for `Blocks`, `Data`, `HeightMap`, and `Biomes` for a seed/dimension/chunk. |
| Public chunk-array generator | Added `worldgen_generate_chunk_arrays()` so tests can generate raw arrays without writing NBT files. |
| Stronghold location API | Added `worldgen_get_stronghold_coords()`, porting the Java 1.2.5 three-stronghold ring placement and `WorldChunkManager.findBiomePosition()` reservoir sampling. |
| Village candidate API | Added `worldgen_village_can_spawn()`, porting Java 1.2.5 grid spacing/separation and plains/desert biome viability check. |
| Mineshaft candidate API | Added `worldgen_mineshaft_can_spawn()`, porting Java 1.2.5 mapgen seed setup and probability check. |
...
| Dump harness | `tools/worldgen_dump.c` now accepts dimensions `0`, `-1`, and `1`. |
...
| Golden comparison tooling | Added `tools/worldgen_compare_hashes.py` to compare `worldgen_dump` output against reference-generated golden CSV hashes. |

### Smoke tests for this pass

```sh
cc -std=c99 -O2 tools/worldgen_dump.c -lm -o worldgen_dump
./worldgen_dump 12345 0 -38 61
./worldgen_dump 12345 0 -156 49
./worldgen_dump 12345 0 -160 -53
./worldgen_dump 12345 -1 0 0
./worldgen_dump 12345 1 0 0
```
