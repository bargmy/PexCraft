...
def write_embedded_mcrw_c(out_assets, embed_assets_c):
    files = sorted([p for p in out_assets.iterdir() if p.is_file() and p.suffix.lower() == '.mcrw'])
    embed_assets_c.parent.mkdir(parents=True, exist_ok=True)
    with open(embed_assets_c, 'w', encoding='utf-8') as f:
        f.write('/* generated by tools/prepare_psp_assets.py; linked into EBOOT.PBP */\n')
        f.write('#include <stddef.h>\n')
        f.write('#ifndef PEX_PSP_EMBEDDED_ASSET_TYPE_DEFINED\n#define PEX_PSP_EMBEDDED_ASSET_TYPE_DEFINED\n')
        f.write('typedef struct PexPspEmbeddedAsset { const char *name; const unsigned char *data; unsigned int len; } PexPspEmbeddedAsset;\n')
        f.write('#endif\n')
        entries = []
        for idx, path in enumerate(files):
            data = path.read_bytes()
            sym = 'pexcraft_psp_asset_%03d' % idx
            f.write('static const unsigned char %s[] = {\n' % sym)
            for i in range(0, len(data), 12):
                f.write('  ' + ', '.join('0x%02x' % b for b in data[i:i+12]) + ',\n')
            f.write('};\n')
            entries.append((path.name, sym, len(data)))
        f.write('const PexPspEmbeddedAsset pexcraft_psp_embedded_assets[] = {\n')
        for name, sym, ln in entries:
            f.write('  {"%s", %s, %du},\n' % (name.replace('\\', '\\\\').replace('"', '\\"'), sym, ln))
        f.write('};\n')
        f.write('const unsigned int pexcraft_psp_embedded_asset_count = %du;\n' % len(entries))
    print(f'embedded PSP MCRW assets: {len(files)} files -> {embed_assets_c}')
...
def main():
    ...
    embed_assets_c = Path(sys.argv[3] if len(sys.argv) > 3 else 'src/assets/psp_embedded_mcrw_assets.c')
