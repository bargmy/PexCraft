/* Standalone Java-1.2.5-style worldgen dump harness.
   Build from repo root:
     cc -std=c99 -O2 tools/worldgen_125_dump.c -lm -o worldgen_125_dump
   Usage:
     ./worldgen_125_dump <seed> <dimension> <chunkX> <chunkZ>
...
int main(int argc, char **argv) {
    ...
    if (!worldgen_125_generate_dimension_chunk_arrays(seed, dim, cx, cz, blocks, data, height, biomes)) return 5;
    ...
    printf("strongholds=%d,%d;%d,%d;%d,%d village_here=%d mineshaft_here=%d\n", sx[0],sz[0],sx[1],sz[1],sx[2],sz[2], worldgen_125_village_can_spawn(seed,cx,cz), worldgen_125_mineshaft_can_spawn(seed,cx,cz));
