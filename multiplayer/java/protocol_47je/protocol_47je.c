...
static void j47_handle_play_packet(int packet_id, J47Reader *r) {
    switch(packet_id){
        ...
        case 0x08:{
            ...
            double current_x=(double)g_player_x,current_y=(double)g_player_y-1.62,current_z=(double)g_player_z;
            if(g_j47.position_received)java47_player_network_position(&current_x,&current_y,&current_z);
            if(flags&1)x+=current_x;else g_player_motion_x=0.0f;
            if(flags&2)y+=current_y;else g_player_motion_y=0.0f;
            if(flags&4)z+=current_z;else g_player_motion_z=0.0f;
            ...
            java47_player_network_position_reset(x,y,z);
            g_player_server_on_ground=0;
