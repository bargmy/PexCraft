/* Android TV classic texture pack installer stub.
...
static void classic_install_fail(const char *msg) {
    lstrcpynA(g_classic_install_error, msg ? msg : "Unsupported on Android TV build", sizeof(g_classic_install_error));
...
static void classic_resource_size_format(char *out, size_t cap) {
    snprintf(out, cap, "not available in this Android TV build");
}

static void start_classic_pack_install(void) {
    classic_install_fail("Runtime client.jar download is disabled for Android TV. Bundle or copy a texture pack instead.");
}
...
static void classic_pack_install_tick(void) {
    ...
    if (state == CLASSIC_INSTALL_ERROR) {
        ...
        open_notice("Texture Pack", "Classic pack download is disabled on Android TV for now.", g_classic_install_error);
    }
}
